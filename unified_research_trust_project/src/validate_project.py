...
def main() -> None:
    ...
    if not 0.15 <= positive_rate <= 0.45:
        raise AssertionError(
            f"Funding rate {positive_rate:.3f} is outside expected range [0.15, 0.45]"
        )
    ...
    if set(routing["routing"].unique()) != expected_routing:
        raise AssertionError(
            "case_routing.csv should contain all three routing categories"
        )
